...
var UpstreamGroup_1 = require("./UpstreamGroup");
...
function nginx(arr) {
    ...
    var res = arr.map(function (item) {
        ...
        if (item.uidArray && item.regionArray) {
            return new three_1.three(item, allServers).doit();
        }
        return item.uidArray || item.regionArray ? new UpstreamGroup_1.UpstreamGroup(item, allServers).doit() : new Upstream(item).doit();
    });
    var content = "" + geo;
    var groups = '';
    var upstreams = '';
